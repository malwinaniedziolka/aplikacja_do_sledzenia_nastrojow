...
function renderChart(type, weekEntries, monthEntries, yearEntries) {
  ...
  if (type === 'week') {
    ...
    labels = grouped.map(e => {
      ...
      return new Date(`${year}-${month}-01`).toLocaleDateString('pl-PL', { month: 'long' }); //Styczeń, Luty ...
    });
  }
...
window.addEventListener('DOMContentLoaded', () => {
  const weekEntries = JSON.parse(document.getElementById('weekData').textContent);
  const monthEntries = JSON.parse(document.getElementById('monthData').textContent);
  const yearEntries = JSON.parse(document.getElementById('yearData').textContent);
